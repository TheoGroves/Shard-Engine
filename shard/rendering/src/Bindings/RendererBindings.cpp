...
PYBIND11_MODULE(shard_render_engine, m)
{
    ...
    py::class_<Engine>(m, "Engine")
        ...
        .def("begin_popup", &Engine::BeginPopup)
        ...
        .def("begin_tooltip", &Engine::BeginTooltip)
        .def("end_tooltip", &Engine::EndTooltip)
        .def("scroll_to_bottom", &Engine::ScrollToBottom)
        .def("get_available_region", &Engine::GetAvailableRegion)
        .def("begin_drag_drop_source", &Engine::BeginDragDropSource)
        .def("end_drag_drop_source", &Engine::EndDragDropSource)
        .def("begin_drag_drop_target", &Engine::BeginDragDropTarget)
        .def("accept_drag_drop_payload", &Engine::AcceptDragDropPayload)
        .def("end_drag_drop_target", &Engine::EndDragDropTarget)
        .def("begin_popup_context_window", &Engine::BeginPopupContextWindow)
        .def("set_drag_drop_payload", &Engine::SetDragDropPayload)
        .def("dummy", &Engine::Dummy)
        .def("begin_menu_bar", &Engine::BeginMenuBar)
        .def("begin_menu", &Engine::BeginMenu)
        .def("end_menu", &Engine::EndMenu)
        .def("end_menu_bar", &Engine::EndMenuBar)
