...
class Material:
    def __init__(self, ctx, tex_path, normal_path, heightmap_path, orm_path, height_scale, uv_scale):
        self.ctx = ctx
        self.texture, self.texture_path = load_texture(ctx, f"assets/textures/{tex_path}" if tex_path else None, "assets/textures/Empty.png")
        self.normal_map, self.normal_map_path = load_texture(ctx, f"assets/textures/{normal_path}" if normal_path else None, "assets/textures/EmptyNormal.png")
        self.heightmap, self.heightmap_path = load_texture(ctx, f"assets/textures/{heightmap_path}" if heightmap_path else None, "assets/textures/EmptyHeightmap.png")
        self.orm_map, self.orm_map_path = load_texture(ctx, f"assets/textures/{orm_path}" if orm_path else None, "assets/textures/EmptyORM.png")
