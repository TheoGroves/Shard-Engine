...
class WarehouseSceneBuilder:
    @staticmethod
    def build(ctx, renderer, shadow_mapper, asset_manager, DEBUG_COLLIDERS, ts: TransformSystem, mrs: MeshRendererSystem, mcs: MeshColliderSystem):
        scene = Scene("Warehouse", renderer, shadow_mapper)
        ts.em = scene.em
        mrs.em = scene.em
        mcs.em = scene.em

        load_many(ctx, asset_manager, shadow_mapper, "assets/models/WarehouseModels", "assets/textures/WarehouseTextures", scene.em, ts, mrs)

        collider_eid, _ = scene.em.create_entity()
